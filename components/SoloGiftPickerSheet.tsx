...
export default function SoloGiftPickerSheet({
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      setBalance(b => b !== null ? Math.max(0, b - totalCost) : null);
      ...
      // Show drop celebration if applicable
  ...
  return (
    <Modal visible={visible} transparent animationType="none" statusBarTranslucent onRequestClose={onClose}>
      ...
      <Animated.View style={[st.sheet, { paddingBottom: insets.bottom + 6, transform: [{ translateY: slideAnim }] }]}>
        ...
        {/* Lucky notice */}
        {isLuckyTab && (
          <View style={st.luckyNotice}>
            <Text style={st.luckyNoticeTxt}>🎰 Lucky Gift — JP reward, no animation effect</Text>
          </View>
        )}
