...
export default function LiveSoloViewerModal({ visible, stream, currentUser, onClose }: Props) {
    ...
    ws.onmessage = (e) => {
      ...
      try {
        ...
        if (p.type === 'LIVE_GIFT' && p.streamId === sid) {
          pushChat({ type: 'gift', username: p.senderUsername ?? 'Viewer', text: `mengirim ${p.giftName} (${p.amountCoins} koin)`, giftEmoji: '🎁', giftCoins: p.amountCoins });
  ...
  const handleGiftSent = useCallback((info: SoloGiftSentInfo) => {
    ...
    launchFloatNotif(
      info.giftEmoji,
      info.giftName,
      currentUser?.displayName ?? currentUser?.username ?? 'Kamu',
      info.price * info.qty,
    );
            ...
            <View style={st.topLeft}>
              <View style={st.hostInfoOverlay}>
                ...
                <View style={{ flexShrink: 1 }}>
                  ...
                  <Text style={st.hostUsername} numberOfLines={1}>
                    @{liveData?.hostUsername ?? '—'}
                  </Text>
                </View>
              </View>
              ...
              if (item.type === 'gift') {
                return (
                  <View style={st.chatBubbleSystem}>
                    ...
                    <Text style={[st.chatUser, { color: ROSE }]}>{item.username} </Text>
                    <Text style={st.chatText}>{item.text}</Text>
                  </View>
                );
              }
